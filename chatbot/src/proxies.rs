...
impl<'a> TwitchChatProxy {
    pub fn new(channel: &str) -> Self {
        ...
        Self {receiver, sender, channel: String::from(channel)}
    }
    ...
    pub async fn initialize(&mut self) -> Result<(), websocket::WebSocketError> {
    ...
    pub fn send_message(&mut self, msg: &str) -> Result<(), websocket::WebSocketError> {
    ...
    pub fn incoming_messages(&mut self) -> impl Iterator<Item = String> + '_ {
        ...
        // let receiver: websocket::receiver::Reader<std::net::TcpStream> = &self.receiver;
        let raw_incoming_messages = self.receiver.incoming_messages();
        raw_incoming_messages.filter_map(|message| match message { 
            Ok(m) => match m {
